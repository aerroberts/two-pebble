...
import type {
  ...
  OpenRouterInputAudioFormat,
  OpenRouterProviderContentBlock,
  OpenRouterProviderMessage,
  OpenRouterProviderOptions,
  OpenRouterProviderRequest,
  OpenRouterProviderRequestToolCall,
  OpenRouterToolPayloadInput,
} from './types';

/**
 * OpenRouter chat completions provider.
 * OpenRouter uses an OpenAI-compatible API with provider-specific headers.
...
export class OpenRouterProvider extends ModelProvider {
  ...
  protected override async invokeProvider(thread: ConversationThread, modelCallId: string) {
    ...
    if (!response.ok) {
      const result: ProviderResult = {
        ...
        startedAt,
        status: 'error',
        error: `OpenRouter request failed with ${response.status}`,
    ...
    const result: ProviderResult = {
      ...
      providerInput: request,
      providerOutput: data,
      prices: buildOpenRouterPriceLineItems({ data, modelId: this.modelId }),
      threadCellPointer,
  ...
  /**
   * Builds the OpenRouter chat completion payload.
   * Tests inspect this without touching the network.
   * Cache turns are sent as user context because OpenRouter has no Pebble cache role.
   */
  public buildRequest(thread: ConversationThread): OpenRouterProviderRequest {
  ...
  private buildMessages(turn: ConversationTurn): OpenRouterProviderMessage[] {
  ...
  private buildAssistantWithToolCalls(turn: ConversationTurn): OpenRouterProviderMessage {
    const toolCalls: OpenRouterProviderRequestToolCall[] = [];
    let text = '';
  ...
  private buildToolResultMessages(turn: ConversationTurn): OpenRouterProviderMessage[] {
  ...
  private buildContentBlocks(cells: DataCells): OpenRouterProviderContentBlock[] {
    const blocks: OpenRouterProviderContentBlock[] = [];
    let textBuffer = '';
  ...
  private audioFormatFor(mimeType: string): OpenRouterInputAudioFormat | undefined {
  ...
  private parseResponse(data: OpenRouterChatCompletionResponse): ProviderOutputBlock[] {
