...
export function isSimpleType(node: ts.TypeNode, options: SimpleTypeOptions): boolean {
  if (isSimpleKeyword(node)) {
    return true;
  }
  if (ts.isTypeReferenceNode(node)) {
    return isSimpleTypeReference(node, options);
  }
  if (ts.isArrayTypeNode(node)) {
    return isSimpleType(node.elementType, options);
  }
...
function isSimpleTypeReference(node: ts.TypeReferenceNode, options: SimpleTypeOptions) {
  if (!isSimpleTypeName(node.typeName)) {
    return false;
  }
...
function isSimpleGenericArgument(node: ts.TypeNode, options: SimpleTypeOptions): boolean {
  ...
  if (ts.isTypeReferenceNode(node)) {
    return node.typeArguments === undefined && isSimpleTypeName(node.typeName);
  }
  if (ts.isArrayTypeNode(node)) {
    return isSimpleGenericArgument(node.elementType, options);
  }
  if (ts.isUnionTypeNode(node)) {
    return (
