...
export class Rule extends Guardrail<TypescriptVariableTypeRuleOptions> {
  ...
  private checkFile(input: TypescriptRuleInput) {
    const visit = (node: ts.Node) => {
      if (ts.isVariableDeclaration(node) || ts.isPropertyDeclaration(node)) {
        this.checkDeclaration(node, input.reporter);
      }
  ...
  private checkDeclaration(node: ts.VariableDeclaration | ts.PropertyDeclaration, reporter: Reporter) {
    if (this.allowsComplexVariableTypes() || this.isGenericBoundary(node) || !node.type) {
      return;
    }
    if (!this.isSimpleType(node.type)) {
      this.fail(reporter);
    }
  }
  ...
  private isSimpleType(node: ts.TypeNode): boolean {
    if (this.isSimpleKeyword(node)) {
      return true;
    }
    if (ts.isTypeReferenceNode(node)) {
      return this.isSimpleTypeReference(node);
    }
  ...
  private isSimpleTypeReference(node: ts.TypeReferenceNode) {
    if (!this.isSimpleTypeName(node.typeName)) {
      return false;
    }
  ...
  private isSimpleGenericArgument(node: ts.TypeNode): boolean {
    ...
    if (ts.isTypeReferenceNode(node)) {
      return node.typeArguments === undefined && this.isSimpleTypeName(node.typeName);
    }
    if (ts.isArrayTypeNode(node)) {
      return this.isSimpleGenericArgument(node.elementType);
    }
    if (ts.isUnionTypeNode(node)) {
      return (
