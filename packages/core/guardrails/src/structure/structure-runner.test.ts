...
import { structureDiagnosticSummary } from './diagnostic-summary';
import {
...
describe('feature: structure guardrails', () => {
  test('happy: passes filesystem and AST rules', async () => {
  ...
  test('unhappy: reports inverted sibling group violations', async () => {
    ...
    expect(structureDiagnosticSummary(root, diagnostics)).toEqual([
      { error: 'structure-rule-failed', file: 'src/example.test.ts' },
      { error: 'structure-rule-failed', file: 'src/index.ts' },
      { error: 'structure-rule-failed', file: 'src/types.ts' },
      { error: 'structure-rule-failed', file: 'src/example-class.ts' },
    ]);
  });

  test('unhappy: reports failed filesystem and AST rules', async () => {
    ...
    const diagnostics = result.results.flatMap((entry) => entry.diagnostics);
    const failure = { error: 'structure-rule-failed', file: '' };

    expect(structureDiagnosticSummary(root, diagnostics)).toEqual([
      { error: 'structure-rule-failed', file: 'src/commented.ts' },
