...
import type { RecordConversationCellInput, RecordModelCallInput, RecordPriceLineItemInput } from './types';
...
export async function recordModelCall(datastore: Datastore, input: RecordModelCallInput): Promise<void> {
  ...
  for (const lineItem of input.call.prices) {
    ...
    input.events.emit('agentPriceLineItemRecorded', lineRecord);
  }
  input.events.emit('agentCallRecorded', record);
}
