...
Set the name by invoking the CLI command:

```
peb agent set-name --agentId {{agentId}} --name "Your Chosen Name"
```

Run this exactly once. Continue with the user's request immediately after. If the command fails, mention it briefly and proceed without retrying.
