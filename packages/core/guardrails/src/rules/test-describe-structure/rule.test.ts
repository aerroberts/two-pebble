...
describe('feature: test-describe-structure rule', () => {
  test('happy: passes focused fixture', async () => {
    const result = await new Controller().run(resolve(import.meta.dirname, 'fixtures/pass'), {
      additional: { '@rule/test-describe-structure': { describeNamePrefix: 'feature: ', allowNestedDescribe: false } },
    });
  ...
  test('unhappy: reports focused violation', async () => {
    ...
    expect(errors).toContain('describe-name');
  });
});
