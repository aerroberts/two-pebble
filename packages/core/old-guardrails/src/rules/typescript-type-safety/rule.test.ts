...
describe('feature: typescript-type-safety rule', () => {
  test('happy: passes focused fixture', async () => {
    const result = await new Controller().run(resolve(import.meta.dirname, 'fixtures/pass'), {
      additional: {
        '@rule/typescript-type-safety': { forbiddenSyntax: ['any', 'unknown', 'satisfies', 'globalThis'] },
      },
    });
  ...
  test('unhappy: reports focused violation', async () => {
    ...
    expect(errors).toContain('type-escape-hatch');
  });
});
