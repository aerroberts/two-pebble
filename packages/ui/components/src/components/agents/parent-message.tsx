...
export function ParentMessageTrace(props: TraceComponentProps<'parent-message'>) {
  ...
  return (
    <AgentTraceItem
      ...
      title={titleForDirection(props.trace.data.direction)}
    />
  );
}
...
function titleForDirection(direction: TraceComponentProps<'parent-message'>['trace']['data']['direction']): string {
  if (direction === 'ask') {
    return 'Parent Agent Ask';
  }
  if (direction === 'response') {
    return 'Parent Agent Response';
  }
  return 'Parent Agent Message';
}
