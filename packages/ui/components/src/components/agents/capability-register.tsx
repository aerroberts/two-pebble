    ...
    <AgentTraceItem
      content={
        <div className="flex flex-col gap-2">
          ...
          <div className="w-full overflow-hidden rounded-md border border-border/60 bg-surface">
            {data.tools.map((tool) => (
              <div
                key={`${tool.type}:${tool.name}`}
                className="flex gap-2 border-t border-border/50 px-2 py-1.5 first:border-t-0"
              >
